...
import cn.distance.module.modules.render.NameTags;
...
import cn.distance.util.render.RenderingUtil;
...
public class Scaffold extends Module {
    ...
    private final MoveUtils move = new MoveUtils();
    ...
    public final Option Timerenable=new Option( "Timer",false );

    public final Option LowBob=new Option( "LowBob",true );

    public final Numbers<Number> TimerValue=new Numbers<Number>("TimerValue",1.0,0.1,10.0,0.1);
    ...
    public Scaffold() {
        ...
        addValues( delayValue,towerValue,towerMoveValue,safeWalkValue,sprintValue,swingValue,LowBob,silentValue,eagleValue,keepYValue,Timerenable,TimerValue);
    }
    ...
    void onUpdate( EventPreUpdate event) {
        ...
        if(this.Timerenable.get()){
            mc.timer.timerSpeed=TimerValue.value.floatValue();
        }
...
//        }
        ...
        if (mc.gameSettings.keyBindJump.isKeyDown() && !MovementUtils.isMoving()
                && !(mc.theWorld
                .getBlockState(new BlockPos(mc.thePlayer.posX, mc.thePlayer.posY - 1.1, mc.thePlayer.posZ))
                .getBlock() instanceof BlockAir)
                && this.towerValue.getValue()) {
    ...
    public void post( EventPostUpdate e) {
        ...
        if (!timer.hasReached(delayValue.getValue().intValue() * 100L))
            return;
        ...
        if (towerMoving()) {
            ...
            if (MovementUtils.isOnGround(1.0E-4)) {
                mc.thePlayer.motionY = MovementUtils.getJumpHeight(0.42);
        ...
        if (this.data != null) {
            if (mc.playerController.onPlayerRightClick(mc.thePlayer, mc.theWorld, mc.thePlayer.getCurrentEquippedItem(),
                ...
                if (sprintValue.get() && mc.thePlayer.onGround ) {
    ...
    public void onRender2DEvent( EventRender2D event) {
        ...
        RenderingUtil.drawOutlinedString(info,
